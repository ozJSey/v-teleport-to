...
imports; `resolve-placement.ts` imports only `types.ts`. `index.ts` is a pure
barrel.
...
5. Always: add a test to `vTeleportTo.test.ts` that pins the contract before
